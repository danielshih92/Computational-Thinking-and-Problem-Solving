...
def draw_estimate_and_sample_normal_distribution(scores, sample_sizes):
    ...
    for i in range(3):
        ...
        plt.plot(x, y, label='μ={:.2f}, σ={:.2f}'.format(sample_means[i], sample_std_devs[i]))
        plt.title('Normal Distribution: μ =  ' + str(round(sample_means[i], 2)) + ', σ = '+ str(round(sample_std_devs[i], 2)))
        ...
        plt.plot([mean, mean], [0, y_mean], color='black', label='μ={:.2f}, σ={:.2f}'.format(mean, std_dev))
        plt.legend()
