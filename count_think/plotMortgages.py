...
def plotMortgages(morts, amt):
    def labelPlot(figure, title, xLabel, yLabel, filename):
        pylab.figure(figure)
        pylab.title(title)
        pylab.xlabel(xLabel)
        pylab.ylabel(yLabel)
    ...
    payments, cost, balance, netCost, enlargedCost = 0, 1, 2, 3, 4
    for i in range(len(morts)):
        ...
        pylab.figure(enlargedCost)
        morts[i].plotTotPd(styles[i])
        pylab.xlim(0, 120)
        pylab.ylim(0,160000)

    labelPlot(payments, 'Monthly Payments of $' + str(amt) +
                ' Mortgages',  'Months', 'Monthly Payments','figure1')
    labelPlot(cost, 'Cash Outlay of $' + str(amt) +
              ' Mortgages', 'Months', 'Total Payments','figure2')
    labelPlot(balance, 'Balance Remaining of $' + str(amt) +
              ' Mortgages', 'Months', 'Remaining Loan Balance of $','figure3')
    labelPlot(netCost, 'Net Cost of $' + str(amt) + ' Mortgages',
              'Months', 'Payments - Equity $', 'figure4')
    labelPlot(enlargedCost, 'Cash Outlay of $' + str(amt) +
              ' Mortgages', 'Months', 'Total Payments', 'figure5')
